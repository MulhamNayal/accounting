...
that codebase's — lazy loading, EF migrations, the repository layer, and error handling. Do
not carry those habits over.

**Architecture:** `Controllers → Services → DbContext → PostgreSQL`. There is **no
repository layer**; services use `ClearWiseDbContext` directly.
...
- Correction links point **backwards only** — `reverses_entry_id` and
...
| Role | Used by | Holds |
|---|---|---|
| `clearwise_owner` | EF migrations, via `ClearWiseDbContextFactory` | DDL |
| `clearwise_app` | The running application | DML only, minus the revokes above |

**The application must never own the ledger tables** — an owner can grant itself back what
...
- **Code-first migrations are the source of truth.** Never hand-edit the schema.
  `dotnet ef migrations add <Name> --project backend/ClearWise.Api/ClearWise.Api.csproj`
- **Lazy loading is OFF.** `.Include()`/`.ThenInclude()` is required, not banned — a
...
- Posting rules are **code, not configuration** — a pure function from document to posting
...
- **Never catch an exception.** `GlobalExceptionHandler` maps type → status in one place:
  `NotFoundException` → 404, `PostingValidationException` → 400,
  `LedgerIntegrityException` → 409, anything else → 502 `ProblemDetails`.
...
- Use `CreatedAtRoute` with an explicit route name, never `CreatedAtAction(nameof(...))` —
...
- Money is `decimal` with explicit precision — `numeric(19,4)` amounts,
  `numeric(19,10)` rates.
...
- **Tests run against a real PostgreSQL**, never an in-memory provider. RLS, deferred
  ...
  implements any of them — a green suite against one would be actively misleading.
...
- **Don't assert the exception type for a deferred constraint** — it surfaces from `COMMIT`
...
- Catch an exception in a controller to map a status — throw a typed exception
...
- Avoid `.Include()` out of lazy-loading habit — lazy loading is **OFF** here
