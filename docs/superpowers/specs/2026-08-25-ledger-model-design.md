...
This specifies the foundation of ClearWise: how a financial fact is stored, corrected,
...
**So ClearWise's differentiator is not features. It is that the books cannot be altered
...
document numbered `#NA`. ClearWise's own numbering is strict; the **importer must not assume
...
| Two incompatible invoice number formats, plus `#NA` | Original numbers are preserved as `external_doc_no`; ClearWise assigns its own. Incoming numbers are treated as opaque text. |
...
| Source documents carry edit history in a change log | Only the current state of each document is imported. The source's audit log is imported, if at all, as read-only reference data — never as ClearWise postings. |
...
2. Decide whether ClearWise's own reporting layer reuses `open-reporting-platform`. If so,
